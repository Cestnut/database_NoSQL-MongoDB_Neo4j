...
def copy_dataset(percentage, debug):
    ...
    for file_name in ["people", "calls", "cells"]:
        input_file_name = file_name + str(percentage) + ".csv"
        output_file_name = file_name + ".csv"
        with open(original_csv+input_file_name, 'r') as input_file,  open(input_csv+output_file_name, 'w') as output_file:
